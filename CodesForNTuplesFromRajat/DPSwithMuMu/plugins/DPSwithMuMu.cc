...
void DPSwithMuMu::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
	...
	if(isMC_) {
		...
		hEvents_MCNLO->Fill(1,genEvt->weight());
	}
	...
	if(isMC_){ 
		...
		for(size_t i = 0; i < genParticles->size() ; ++ i){
			...
			if( ( fabs(q.pdgId()) >=  1 && fabs(q.pdgId()) <=6)  || (fabs(q.pdgId())== 21) ){
		...
		try {iEvent.getByToken(theLHEPTag, lheEventProduct);} catch (...) {;}
	...
	trigDecision->insert(trigDecision->begin()+3,0);
	...
	for(unsigned i=0;i<names.size();i++){
		...
		if((names.triggerName(i).find("HLT_IsoMu22_v") != npos) || (names.triggerName(i).find("HLT_IsoTkMu22_v") != npos)) {
			if(triggerBits->accept(i)){
				trigDecision->insert(trigDecision->begin()+3,1);
				fired4=true;
			}
		}
	...
	} // fill info when trigger 2 is fired
	...
	for (unsigned int i=0;i<MuonVector.size();i++){
		...
		if(isMedium &&  MuonVector[i]->pt() > 10.0 && fabs(MuonVector[i]->eta()) < 2.4 && iso_val < 0.15){ 
			...
			T_mu_trkLayers.push_back(MuonVector[i]->innerTrack()->hitPattern().trackerLayersWithMeasurement() );
	...
	for(edm::View<pat::Jet>::const_iterator iJet = patjets->begin();iJet != patjets->end(); ++iJet){
		if(iJet->pt()>10.0){
			...
			double pt_jets=0, pt_jets_tot=0, pt_jets_pu=0;
			const unsigned int nPFCands = iJet->numberOfDaughters();
			for(unsigned int ipf=0; ipf<nPFCands; ipf++) {
				const pat::PackedCandidate *cand = dynamic_cast<const pat::PackedCandidate* >( iJet->daughter(ipf) );
				pt_jets_tot += cand->pt();

				if(fabs(cand->dz(vertex->position())) < 0.2) {
					pt_jets += cand->pt();
				}
			...
			jet_total_pt.push_back(pt_jets_tot);
			jet_sig_pt.push_back(pt_jets);
			jet_pu_pt.push_back(pt_jets_pu);


			float matchgenjetpx;
			float matchgenjetpy;
			float matchgenjetpz;
			float matchgenjet_energy;

			matchgenjetpx = -999;
			matchgenjetpy = -999;
			matchgenjetpz = -999;
			matchgenjet_energy = -999;

			if(isMC_) {
				if(iJet->genJet()) { const reco::GenJet* genJet=iJet->genJet(); 

					matchgenjetpx = genJet->px();
					matchgenjetpy = genJet->py();
					matchgenjetpz = genJet->pz();
					matchgenjet_energy = genJet->energy();
				} }

			T_genjet_match_px.push_back(matchgenjetpx);
			T_genjet_match_py.push_back(matchgenjetpy);
			T_genjet_match_pz.push_back(matchgenjetpz);
			T_genjet_match_energy.push_back(matchgenjet_energy);
			...
			T_Jet_puMVA.push_back(iJet->userFloat("pileupJetId:fullDiscriminant") );
...
DPSwithMuMu::beginJob()
{
	...
	mytree_->Branch("lheHt",&_lheHt,"lheHt/F");  
	mytree_->Branch("lheNOutPartons", &_lheNOutPartons, "lheNOutPartons/I");
	mytree_->Branch("lheNOutB", &_lheNOutB, "lheNOutB/I");
	mytree_->Branch("lheNOutC", &_lheNOutC, "lheNOutC/I");
	...
	mytree_->Branch("T_trg1_Pt", "std::vector<float>", &trg1_Pt);
	mytree_->Branch("T_trg1_Eta", "std::vector<float>", &trg1_Eta);
	mytree_->Branch("T_trg1_Phi", "std::vector<float>", &trg1_Phi);
	mytree_->Branch("T_trg1_Energy", "std::vector<float>", &trg1_Energy);

	mytree_->Branch("T_trg2_Pt", "std::vector<float>", &trg2_Pt);
	mytree_->Branch("T_trg2_Eta", "std::vector<float>", &trg2_Eta);
	mytree_->Branch("T_trg2_Phi", "std::vector<float>", &trg2_Phi);
	mytree_->Branch("T_trg2_Energy", "std::vector<float>", &trg2_Energy);

	mytree_->Branch("T_jet_total_pt","std::vector<float>", &jet_total_pt);
	mytree_->Branch("T_jet_sig_pt","std::vector<float>", &jet_sig_pt);
	mytree_->Branch("T_jet_pu_pt","std::vector<float>", &jet_pu_pt);

	mytree_->Branch("T_genjet_match_px","std::vector<float>", &T_genjet_match_px);
	mytree_->Branch("T_genjet_match_py","std::vector<float>", &T_genjet_match_py);
	mytree_->Branch("T_genjet_match_pz","std::vector<float>", &T_genjet_match_pz);
	mytree_->Branch("T_genjet_match_energy","std::vector<float>", &T_genjet_match_energy);
	mytree_->Branch("T_Jet_puMVA","std::vector<float>", &T_Jet_puMVA);
	...
	mytree_->Branch("T_mu_trkLayers", "std::vector<float>", &T_mu_trkLayers); 
...
void DPSwithMuMu::endEvent(){
	...
	T_genjet_match_px.clear();
	T_genjet_match_py.clear();
	T_genjet_match_pz.clear();
	T_genjet_match_energy.clear();
